...
**表の中で段を上げない。**`<Table>` が `text-xs`（12px）を持つので、セルやその中の
ボタンに `text-sm` を書くと**その列だけ文字が大きく見える**。2026-09-07 に収支台帳の
点数と購入表の買い方で実際にそうなっていた。目立たせたいならアイコンか太さで、
大きさは変えない。
...
**枚数は「答えの数」で決める。**指標の数ではない。答えが 1 つに絞れる画面は
1 枚、絞れない画面は**対で読む指標をひとまとまり**にして並べる。

| 画面 | 枚数 | なぜ |
|---|---|---|
| 収支台帳 / シミュレーションの結果 | **5 枚**（投資 / 払戻 / 損益 / 回収率 / 的中率） | 1 つを答えに選べない。投資と払戻は対、回収率と的中率も対（的中率だけ高くても配当が小さければ負ける） |
| モデル詳細 | **5 枚**（単複の回収率・的中率 / log-loss） | 回収率は単勝と複勝が対、的中率も同じ。log-loss は市場との比較で初めて意味を持つ |
| モデル画面「いま予想に使っているモデル」 | **役割ごとに 1 枚の計 2 枚** | 役割が 2 つあり、それぞれに答えが 1 つずつある。以前 7 枚並べていたのを絞った |

**5 枚を並べるときは並びと言い回しを揃える。**収支台帳とシミュレーションの結果は
同じ 5 つを同じ順・同じラベルで出す。「この買い方を続けたら」と「実際に続けたら」を
見比べる画面なので、並びが違うと突き合わせられない。

**それ以外の値はカードにしない。**`Figures`（ラベルの下に値を置くだけ・枠も面も
無し）に落とす。シミュレーションの「必要だった資金」は台帳に無い量なので 5 枚には
入れず、下に添えてある。

カードの中身は **値・判定・窓の 3 段**にできる — この 3 つはバラすと読めない
（値だけでは基準が分からず、判定だけでは根拠が分からず、窓が無いとどちらも意味を
持たない）ので、囲いはその 3 つを 1 つに束ねる仕事をしている。
