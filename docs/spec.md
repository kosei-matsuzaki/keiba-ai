...
**`from` / `to` はどの bets 系エンドポイントでも「確定日」で絞る**（`settled_at`。
未確定の買い目は記録日 `created_at` で拾う）。`to` は日付だけで来るので `T23:59:59` を
足してから比べる — 足さないと `2026-09-07T04:21` が `2026-09-07` より大きく、
**その日に記録したぶんが必ず落ちる**。以前 `GET /api/bets` だけ `created_at` を素で
比べていて、同じ画面の同じ日付フィルタでサマリ 118 件・明細 0 件になっていた。
