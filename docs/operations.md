...
uv run python -m ai.evaluate --model data/models/20260501-120000

# 評価結果を model_runs.metrics_json に保存する（Dashboard MetricCard に反映させる場合は必須）
uv run python -m ai.evaluate --model data/models/20260501-120000 --persist
...
# メモリ上のみ（中断不可）
uv run python -m ai.gbm.tune --n-trials 50

# SQLite に保存（中断・再開が可能）
uv run python -m ai.gbm.tune \
    --n-trials 50 \
...
2. `uv run python -m ai.gbm.train --params-json configs/production_lgb.json --train-end $(date +%Y-%m-%d)` で再学習する
3. バックテスト評価で指標を確認する
...
active モデルが存在しません。`GET /api/models` でモデル一覧を確認し、`POST /api/models/{id}/activate` で active モデルを設定してください。モデルがなければ `uv run python -m ai.gbm.train` で学習します。
...
**症状**: `uv run python -m ai.gbm.train` がエラーまたは異常終了する
