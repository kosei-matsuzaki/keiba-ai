...
2. `uv run python -m ai.nn.train_nn --loss multi --monitor valid_tansho_roi --init-from <事前学習ts>-nn --train-end $(date +%Y-%m-%d)` で再学習する
3. バックテスト評価で指標を確認する
...
active モデルが存在しません。`GET /api/models` でモデル一覧を確認し、`POST /api/models/{id}/activate` で active モデルを設定してください。モデルがなければ `uv run python -m ai.nn.train_nn` で学習します。
...
**症状**: `uv run python -m ai.nn.train_nn` がエラーまたは異常終了する
