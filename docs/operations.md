...
# デフォルトパラメータで学習（min_data_in_leaf=5 の synthetic-friendly 設定）
uv run python -m keiba_ai.ai.train

# 本番パラメータで学習（min_data_in_leaf=50 の大規模データ向け設定）
uv run python -m keiba_ai.ai.train \
    --params-json configs/production_lgb.json \
...
uv run python -m keiba_ai.ai.evaluate --model data/models/20260501-120000

# 評価結果を model_runs.metrics_json に保存する（Dashboard MetricCard に反映させる場合は必須）
uv run python -m keiba_ai.ai.evaluate --model data/models/20260501-120000 --persist
...
### Optuna ハイパーパラメータチューニング（オプション）

十分な実データが蓄積された後に任意で実施します。

```bash
cd backend

# メモリ上のみ（中断不可）
uv run python -m keiba_ai.ai.tune --n-trials 50

# SQLite に保存（中断・再開が可能）
uv run python -m keiba_ai.ai.tune \
    --n-trials 50 \
    --storage sqlite:///data/tune.db
```

チューニング完了後、最良パラメータを JSON に書き出して `--params-json` に指定し本番学習を再実行します。
...
backend/data/models/
├── 20260501-120000/
│   ├── model.txt        # LightGBM Booster のテキストシリアライズ形式
│   └── meta.json        # params / train_range / valid_range / metrics / feature_columns
├── 20260601-090000/
│   └── ...
```
...
`configs/production_lgb.json` は大規模データ向けのパラメータセット（`num_leaves=127`、`learning_rate=0.03`、`min_data_in_leaf=50`、`num_boost_round=2000`）です。

評価指標を確認し、既存の active モデルより改善していれば active に切り替えます。
...
2. `uv run python -m keiba_ai.ai.train --params-json configs/production_lgb.json --train-end $(date +%Y-%m-%d)` で再学習する
3. バックテスト評価で指標を確認する
...
`uv sync` で SQLAlchemy / Alembic / LightGBM / pandas / numpy / scikit-learn / shap / optuna / FastAPI / uvicorn / pydantic-settings を含む全依存関係がインストールされます。`alembic upgrade head` を実行すると `data/keiba.db` に全 8 テーブルが作成されます。
...
# 別ポートを使う場合
KEIBA_API_PORT=9000 uv run python -m keiba_ai.main
...
**症状**: `uv run uvicorn keiba_ai.main:app --port 8765` がエラーで起動しない
...
active モデルが存在しません。`GET /api/models` でモデル一覧を確認し、`POST /api/models/{id}/activate` で active モデルを設定してください。モデルがなければ `uv run python -m keiba_ai.ai.train` で学習します。
...
| 原因 | 確認・対処 |
|---|---|
| バックエンドが起動していない | 別ターミナルで `cd backend && uv run uvicorn keiba_ai.main:app --host 127.0.0.1 --port 8765 --reload` を実行する |
| ポートが異なる | `VITE_KEIBA_API_BASE_URL=http://127.0.0.1:<port> pnpm dev` で正しいポートを指定する |

### LightGBM 学習エラー

**症状**: `uv run python -m keiba_ai.ai.train` がエラーまたは異常終了する

| 原因 | 確認・対処 |
|---|---|
| `min_data_in_leaf=50`（本番推奨値）で小規模データだと学習できない | `--params-json` で `min_data_in_leaf` を小さい値（例: 5）に設定する。デフォルトは `5` |
| valid_df のみが空になる（`--valid-months 0` など） | early stopping を諦め `num_boost_round` 上限まで学習する。test セットは独立して保持されるためスコアは正常。ただし過学習リスクがあるため本番では非推奨 |
| train_df まで空になる | 全データ（test 含む）でフォールバック学習が発動する。この場合 test_ndcg が 1.0 に近い偽スコアになる可能性がある。test_ndcg ≈ 1.0 が出たら data leak を疑い、データ量を確認するか `--valid-months` / `--test-months` を短く設定する |
...
| グループサイズ 1 のレースで NDCG 計算失敗 | 1 頭しか出走しないレースが含まれている。実データでは発生しないが、synthetic データ生成時はグループサイズを 2 以上にする |
...
本番モデル（`num_boost_round=2000`、`num_leaves=127`）では SHAP の `TreeExplainer` 計算が小規模モデルより遅くなります。`GET /api/predictions/{race_id}` の応答時間が問題になる場合は、馬数が多いレースでの動作を事前に確認してください。
