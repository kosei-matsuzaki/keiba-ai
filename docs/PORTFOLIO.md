...
|---|---|
| 開発期間 | 2026年3月〜（継続開発中） |
| 規模 | Python 約550コミット / backend テスト 1,000+ 件 / frontend テスト 190+ 件 |
...
**工学面の学び**: 時系列データのリーク防止設計、確率的予測の校正 (proper scoring rule)、非同期ジョブ管理、SQLite の実運用（WAL/TRUNCATE、破損対策、世代バックアップ）、560 コミットにわたる単独開発でのドキュメント駆動の仕様管理。
