...
**単勝には効かない。** オッズ帯 10〜25 の中で PL の確率が上下を分けられるかを測ったが、
**同じ PL モデルの版違いで符号が反転した**:

| 帯の中 (holdout, n=404) | 上位半分 | 下位半分 | 差 |
|---|---|---|---|
| 学習が 2 年短い PL | 1.081 | 1.763 | −0.68 |
| 正しい分割の PL | 1.510 | 1.305 | **+0.205** |

どちらも対照のばらつき（目的変数をシャッフルした 20 回の差の σ=0.53）の内側で、
**帯の中では信号が無い**と結論する。最初に見た「逆向き」は解釈すべきではなかった。
帯を細分する材料は、少なくとも holdout 404 レース・的中 40 本では得られない。

### 組み合わせ

2 つのルールが選ぶレースは**完全に排他**（重なり 0 レース）:

| | 単勝（本命が 10〜25 倍） | 複勝（PL 確信度 上位25%） | 総合 |
|---|---|---|---|
| dev | 388点 1.419 | 670点 0.945 | **1.113** |
| holdout | 404点 1.409 | 723点 0.945 | **1.107** |
| 全レース買う | 2,679点 | 2,679点 | 0.929 / 0.893 |

ただし **総合 1.107 は「単勝の帯だけ買う 1.409」より低い**。複勝の脚が 1.0 未満なので
混ぜると薄まる。複勝を足す意味は分散にある（的中率 10% の 404 点に、的中率 77% の
723 点を足すと資産曲線が安定する）。どちらを採るかは運用の判断で、モデルの問題ではない。

検証ツール: `scripts/ensemble_signal_dump.py` → `scripts/ensemble_analysis.py`。
