...
- **連系の確率を PL 由来にする**: 連系の確率は NN スコアから**解析的 Plackett-Luce** で
  導出しているが、`log_growth` 系で学習したスコアには「PL の強度パラメータである」保証が
  無く、**導出の前提と学習の目的が食い違っている**。単勝で確認された確率の破綻
  （相関 0.073）は、複数頭のスコア差の積で決まる連系ではより悪化するはず。
  `plackett_luce` で学習したモデルの確率に差し替えれば、EV 判定が初めて機能する可能性がある
  （買い目の候補は active のまま、確率だけ差し替える = 複勝で効いた役割分担と同じ形）
