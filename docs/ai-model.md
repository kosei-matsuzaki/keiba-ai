...
| レースの選別 | AI 予想は**全レース**で出す。シミュレーションのみ「厳選」戦略で、本命が 10〜25 倍のレースに絞れる（→ [戦略レイヤー](#戦略レイヤー--どのレースを買うか2026-08-24)）|
...
単複とも市場ベースラインを上回るが、**全レースを買う限りどちらも 1.0 未満**（控除率の壁の
内側で最適化されただけで黒字ではない）。特徴量・損失・モデル側 overlay の 3 方向はいずれも
実測で否定済み。一方で**買うレースを選ぶ**方向には効き目があり、本命が 10〜25 倍のレースに
限ると単勝 1.414（792 レース、dev / holdout とも 1.4 台）になる
（→ [戦略レイヤー](#戦略レイヤー--どのレースを買うか2026-08-24)）。
...
## 戦略レイヤー — どのレースを買うか（2026-08-24）

モデルを固定したとき、総合回収率は「レースごとの回収率の、賭け金による加重平均」なので、
動かせるレバーは **(1) どのレースを買うか（買わない判断を含む）** と
**(2) どのレースに厚く賭けるか** の 2 つしかない。どちらも効くのは
「**賭ける前に観測できる何か**がそのレースの回収率と相関している場合」だけで、
相関が無ければ配分をどう組んでも平均は動かない（分散が変わるだけ）。
新馬戦の除外が効かなかったのはこの相関が無かったため。

### 測り方

test 期間 5,390 レース（`20260613T114817-nn` の学習に一切使っていない 2024-11-02〜2026-05-31）
について、**賭ける前に分かる signal** と **実際のリターン**を 1 レース 1 行で書き出し
（`scripts/strategy_signal_dump.py`）、**前半 dev で探索・後半 holdout で検証**した
（`scripts/strategy_slice_analysis.py`）。5,000 レースに数十通りの切り口を当てれば
「たまたま回収率 1.0 超え」の帯は必ず見つかるので、この分離が無いと結論が出せない。

### 出てきたもの: 本命のオッズ帯

| モデルの本命のオッズ | レース数 | 的中 | 単勝回収率 | 95% CI | dev | holdout |
|---|---|---|---|---|---|---|
| 全レース | 5,390 | — | 0.933 | [0.86, 1.01] | 0.97 | 0.90 |
| 8〜25 倍 | 1,111 | 107 | 1.281 | [1.03, 1.55] | 1.34 | 1.22 |
| **10〜25 倍** | **792** | **72** | **1.414** | **[1.10, 1.74]** | **1.42** | **1.41** |
| 15〜25 倍 | 360 | 35 | 1.860 | [1.29, 2.47] | 2.00 | 1.73 |
| 25 倍以上 | 276 | 6 | 0.78 | — | 0.76 | 0.80 |

信頼区間が 1.0 を跨がず、dev と holdout がほぼ一致し、半年ごとに割っても
1.29 / 1.55 / 1.31 / 1.45 と全期間で 1.0 超え。最大配当を除いた jackknife でも 1.6 台を保つ。

**メカニズムも説明がつく。** 同じ 15〜25 倍帯の全出走馬（8,761 頭）の市場全体の勝率は 4.60%
（回収率 0.879）なのに対し、**モデルの本命がこの帯に来たときの勝率は 9.7%** と倍ある。
つまりモデルは「市場が過小評価している中穴」を当てられていて、その edge は
**市場と食い違うレースにしか無い**。本命が 1 番人気のレース（全体の 36%）は回収率 0.855 で、
控除率に負けている。人気順で切っても同じ構造が出る（本命が 6〜8 番人気: 1.350、
dev 1.29 / holdout 1.40）。

**EV は使い物にならない**ことも同時に確認できた: EV 1.0〜1.3 → 0.970、EV 1.3〜2.0 → 0.839、
EV 2.0 以上 → 1.111 と単調ですらない。オッズ帯の方が桁違いに素直な signal で、
「EV が 1.0 を超えたら買う」という全券種共通の閾値には根拠が無い。

### 実装（シミュレーション限定）

`STRATEGY_PRESETS` に `selective`（UI では「厳選」）を追加した。balanced に加えて
**本命のオッズが 10〜25 倍のレースでだけ単勝・複勝を買う**（連系は自前の EV 条件のままで、
このフィルタは通さない）。既定は balanced のままで、**AI 予想（`ai/betting/strategy.py`）には
入れていない**。

**理由**: この帯は**確定オッズ（= 払戻）で切っている**が、実際に賭けるのは締切前で、
そのときのオッズは確定値と違う。境界のレースは実運用とズレるので、まず
シミュレーションで数字を貯めてから本番に入れる順序にしてある。締切前オッズの
時系列は現状 DB に無い（`odds.db` は確定オッズのみ）ため、ズレの大きさは未測定。

その他の注意:

- 的中は 72 本しかない。回収率 1.41 でも 10 レースに 1 回しか当たらず、ドローダウンは深い
- 単勝を買うのは全レースの **15%** だけになる（年 500 レース程度）
- 複勝は同じ帯で 1.023 [0.90, 1.15] と損益分岐付近（全体は 0.887）。有意ではない
- 1 モデル・19 ヶ月の結果。**モデルを学習し直したら測り直すこと**
