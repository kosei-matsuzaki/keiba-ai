...
同一分割（train 〜2024-04-28 / valid 6ヶ月 / test 19ヶ月）で損失だけを替えて比較した
（`20260824T173853-nn`）:

| | active (`multi`) | 新規 (`plackett_luce`) |
|---|---|---|
| NDCG@3 | 0.5104 | **0.5844** |
| 本命の的中率 | 0.2306 | **0.3053** |
| 単勝回収率 | **0.9303** | 0.8165 |
| 複勝回収率 | **0.8857** | 0.8788 |
| 本命の確率と勝敗の相関 | 0.073 | **0.277** |
| 本命の確率の log-loss | 0.557 | **0.504** |

**PL は勝ち馬をよく当てるが儲からない**（人気馬を選ぶので配当が小さい）。逆に `multi` は
当てないが儲かる（値付けの歪みを突く）。両者が同じ馬を本命にするのは **34.7%** だけで、
持っている情報が違う。
...
| 上位 50% | 0.916 | 0.921 | [0.883, 0.960] | 0.675 |
| 上位 25% | 0.912 | 0.938 | [0.896, 0.980] | 0.766 |
| 上位 9% | 0.933 | 0.952 | [0.898, 1.004] | 0.823 |

dev / holdout の両方で単調に上がる。黒字には届かないが、**再現する確信度の指標**である。

**単勝には効かない。** オッズ帯 10〜25 の中では逆向きですらある（PL の確率が下位半分の
ほうが 1.763 で、上位半分の 1.081 より高い）。帯の edge は市場との食い違いから来ており、
市場寄りの PL が同意しないほど食い違いが大きい、と読める。ただし差 0.68 は対照の
ばらつき（σ=0.53）と同程度なので**示唆であって確証ではない**。
...
|---|---|---|---|
| dev | 388点 1.419 | 670点 0.912 | **1.094** |
| holdout | 404点 1.409 | 701点 0.938 | **1.107** |
...
701 点を足すと資産曲線が安定する）。どちらを採るかは運用の判断で、モデルの問題ではない。
