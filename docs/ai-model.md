...
| 連系計 | 0.849 → 0.877 | — | EV 撤廃 + 確率モデル由来の確率。**ただし前進検証 9 fold では差が検出できない**（0.832 vs 0.839） |
...
**本番（with-odds）の tansho ROI はどれも改善しない**。harness は `scripts/model_side_ab.py`。
...
既定の単勝ルール（`--win-bet-rule top1`）が使う `--win-min-odds`（既定 1.1）は
...
| **連系** | 上位 `top_n_horses` 頭から組み、的中確率の高い順に **券種ごと `max_points_per_bet_type` 点まで**（予算は上限であって使い切らない） |
...
#### 連系は予算まで買い切らない（2026-09-01）

**予算は上限であって、使い切る目標ではない。** 買い目は的中確率の高い順に並び、
その順序は本物（実測 5,404 レース: 1 点目の的中率 0.155 → 10 点目 0.037）。深く買うほど
当たりにくい買い目に金を足すことになる。

| 券種ごとに何点まで | 連系 ROI | 連系に使う金 |
|---|---|---|
| 上位 1 点 | 0.903 | 25% |
| **上位 2 点（既定）** | **0.898** | **44%** |
| 上位 3 点 | 0.872 | 59% |
...
半期に割ると 4 期中 3 期でプラス、1 期（2024-H2・551 レース）で −0.145 と揺れるので
「回収率が上がる」とまでは言い切れない。確かなのは**同じかやや良い成績を半分以下の金で
得られる**こと。設定は `max_points_per_bet_type`（既定 2、0 で無制限）。単勝・複勝は
元から 1 点なので影響しない。
...
賭け金を確信度で動かしてよいのは複勝だけ（連系も無相関: 0.877 → 0.879）。
