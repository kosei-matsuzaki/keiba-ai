...
1. 期間 — **「全期間」と「カスタム」だけ。**日数の窓（直近 7 日 / 30 日）は置かない
2. 収支 — **`MetricCard` 5 枚**（累計投資 / 累計払戻 / 純利益 / 回収率 / 的中率）。
   シミュレーションの結果と同じ並び・同じ言い回し
3. 損益推移 — 0 起点。**シミュレーションと同じ `ProfitChart`** を使う
4. 内訳 — 馬券種別 / レース格別 / 月別を**タブで切り替える**（同じ形の表を積むと
   縦に伸びるだけで見比べられない）。`source` はタブに置かない — 上の
   「全件 / AI推奨 / 手動」フィルタと同じ軸で、2 か所から操作できると読めなくなる
5. 購入記録テーブル — `AddBetDialog` で手動登録 / CSV 書き出し

**日数の窓を置かない。**JRA は土日開催なので 7 日 = 1〜3 開催日にしかならず、画面の
主役である回収率がそこでは読めない。実測（2025-09〜2026-02・7,006 点）で直近 7 日は
**1.073**、全期間は **0.835** — 短い窓だけが「黒字」に見えた。0.05 の差を言うのに
複勝で 1.6 年ぶんが要る量なので（[ai-model.md](ai-model.md)「OOS 実測」）、日数の窓は
嘘をつく側にしか働かない。先週末だけ見たいときはカスタムで日付を入れる。

**買っていない日を損益推移の点にしない。**同じ理由の裏返しで、土日開催だと日次の
8 割以上が値を持ち越すだけの平日になり（12ヶ月 372 点中、買った日は 53 点）、線が
階段状に潰れる。落とすと 5 日の間隔と 1 日の間隔が同じ幅になるが、累計損益は買った
回数の積み上げなので、開催日を等間隔に並べるほうが曲線として読める。
