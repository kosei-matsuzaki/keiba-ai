...
netkeiba データを NN（PyTorch Set Transformer ランキング）で学習し、単勝・複勝の買目予想と確率を提示する個人研究用ツール。FastAPI バックエンド + React 管理画面構成。`scripts/dev.sh` で uvicorn + Vite を一発起動し、ブラウザでアクセスする。
...
| ファイル | 概要 |
|---|---|
| [spec.md](spec.md) | 技術仕様（技術スタック・ディレクトリ構成・DBスキーマ・API エンドポイント・開発ビルド手順） |
...
| [ai-model.md](ai-model.md) | モデル設計（問題定義・Set Transformer・損失（multi/PL）・確率変換・特徴量・学習評価フロー・ベットルール） |
| [operations.md](operations.md) | 運用（ローカル開発セットアップ・データ取り込み運用・再学習サイクル・モデル世代管理・バックアップ・トラブルシューティング） |
