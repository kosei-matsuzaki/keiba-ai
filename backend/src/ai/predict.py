...
SHAP TreeExplainer は GBDT 限定だったため、NN では predict_race_with_shap が
top_features=[] を返す。
"""
...
def predict_race_with_shap(
    ...
    SHAP TreeExplainer は GBDT 限定だったため、NN モデルでは top_features に
    空リストを入れて返す (UI 側で「説明なし」表示にできる)。``top_n`` は
    シグネチャ互換のため残すが未使用。
