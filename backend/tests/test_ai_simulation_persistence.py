...
from ai.simulation.engine import GroupStats, ProfitPoint, SimulationResult
...
def _make_result(race_budget: int = 5_000, final: int = 5_000) -> SimulationResult:
    ...
    return SimulationResult(
        ...
        profit_timeseries=[
            ProfitPoint(date="2024-01-01", profit=500, invested=100, payout=600, n_bets=1),
            ProfitPoint(date="2024-01-02", profit=1_000, invested=100, payout=600, n_bets=1),
        ],
    )
...
def test_save_simulation_result_creates_row(engine):
    with Session(engine) as session:
        ...
        result = _make_result(race_budget=5_000, final=1_000)
        ...
        assert saved.race_budget == 5_000
        assert saved.final_profit == 1_000
        parsed_summary = json.loads(saved.summary_json)
        assert parsed_summary["n_bets"] == 20
        parsed_ts = json.loads(saved.profit_timeseries_json)
...
def test_save_simulation_result_prunes_when_over_limit(engine):
    ...
    with Session(engine) as session:
        ...
        for i in range(MAX_SAVED_RUNS + 5):
            result = _make_result(race_budget=5_000 + i, final=0)
            saved = save_simulation_result(session, result, model_id)
        ...
        # 予算の小さい (= 古い) 5 件は消えていて、新しい 50 件が残る
        budgets = sorted(r.race_budget for r in runs)
        assert budgets[0] >= 5_000 + 5
...
def test_list_simulation_runs_orders_newest_first(engine):
    with Session(engine) as session:
        ...
        for i in range(3):
            saved = save_simulation_result(
                session, _make_result(race_budget=100 * (i + 1)), model_id
            )
    ...
    assert len(runs) == 3
    # 新しい順
    assert runs[0].race_budget == 300
    assert runs[1].race_budget == 200
    assert runs[2].race_budget == 100
...
def test_list_simulation_runs_filters_by_model(engine):
    ...
    with Session(engine) as session:
        ...
        save_simulation_result(session, _make_result(race_budget=5_000), model_a)
        save_simulation_result(session, _make_result(race_budget=5_000), model_a)
        save_simulation_result(session, _make_result(race_budget=5_000), model_b)
        session.commit()
...
def test_api_list_then_get_then_delete(api_client: TestClient):
    ...
    assert body["race_budget"] == 5_000
    assert body["final_profit"] == 5_000
    # profit_timeseries が JSON decode されて含まれる
    assert len(body["profit_timeseries"]) == 2
